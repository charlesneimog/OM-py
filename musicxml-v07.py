...
def fix_nested_tuplets(tuplet):
    ...
    max_level_of_this_group = max(list(map(lambda x: x[1].tuplet_level, tuplet)))
    min_level_of_this_group = min(list(map(lambda x: x[1].tuplet_level, tuplet)))
    index = 0
    
    
    
    for x in tuplet:
        print('Total duration', (x[1].minum_value / x[1].total_duration))
        ...
        if x[1].tuplet_level == 1:
            if x[1].min_level_of_tuplets != 0:
                actual_pulses.append(pulse)
                if last_item == x:
                    for pulses in actual_pulses: 
                        final_tree.append(pulses)
                        index += 1
            else:               
                if x[1].all_measures_dots == 0:
                    ...
                    if x[2] == 'stop':
                        ...
                        tree = [tree]
                        duration_of_tree = (x[1].total_duration * fix_when_have_dots)
