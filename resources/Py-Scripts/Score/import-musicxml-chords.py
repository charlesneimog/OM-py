...
for part_index in xml_data.parts:
    technique_value = 'ord.'
    Instrument = part_index.partName
    ...
    for measure in all_measures:
        ...
        for notes_and_rests in measure.notesAndRests: ### Todos os valores ritmicos (eu acho)
            ...
            there_is_tie = notes_and_rests.tie
            tied = False
            if there_is_tie != None:
                tie_value = there_is_tie.type
            else:
                tie_value = None
            if there_is_tie != None or houve_tie:   
                if tie_value == 'start':
                    tied = False
                    houve_tie = True
                else:
                    if houve_tie and tie_value != 'start': # Prevent error from Dolet 8.0
                        tied = True
                        houve_tie = False
                    else:
                        tied = False
                        
            if notes_and_rests.isRest or tied: 
                None # Se a nota e pausa ou e ligada nao se deve salvar os valores!
                ...
                if isinstance(notes_and_rests, music21.note.Note):
                    ...
                    # ======================================== articulations ==================================       
                    # =========================================================================================
                    if notes_and_rests.articulations != []:
                        articulations = list(map(lambda x: x.name, notes_and_rests.articulations))
                    else:
                        articulations = []
                    
                    if notes_and_rests.expressions != []:
                        haste_articulations = (list(map(lambda x: x.name, notes_and_rests.expressions)))
                        articulations = articulations + haste_articulations

                    if articulations == []:
                        articulations = []

                    ALL_DATA.append(articulations)    
                    ...
                    for note in notes_and_rests.notes:
                        ...
                        if notes_and_rests.articulations != []:
                            ...
                            articulations = None    
                        PITCH_DATA.append(articulations)
                        # ======================================== variables ======================================       
                        # =========================================================================================
                        VAR = remove_the_var(variaveis, 'c')
                        if VAR == []:
