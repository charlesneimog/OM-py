...
(probe-file (format nil "~{~a~}" (mapcar (lambda (x) (om::string+ x "/")) (butlast file))))))
...
(let* (
      (build (om::om-cmd-line (format nil "~d && python ~d bdist_wheel" om-py::*activate-virtual-enviroment* (namestring module))))
      (thepath (om::string+ (namestring (get-folder module)) "/dist/"))
      ; search for the wheel file in the dist folder
      (thefilelist (om::om-directory thepath 
                                    :type :all
                                    :directories t 
                                    :files t 
                                    :resolve-aliases nil 
      ...
      (loop :for loop-files :in thefilelist 
            :do (om::om-cmd-line (format nil "~d && pip install ~d" om-py::*activate-virtual-enviroment* (om::string+ thepath loop-files)))))) 
